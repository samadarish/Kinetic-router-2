...
export function CopyPageButton() {
  ...
  async function copy() { const text = document.querySelector('.docs-article article')?.textContent ?? ''; if (!navigator.clipboard) return; await navigator.clipboard.writeText(text); trackDocsCopy(); setCopied(true); setTimeout(() => setCopied(false), 1200); }
