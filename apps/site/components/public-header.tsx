  ...
  return (
    <>
      <nav className="fixed inset-x-0 top-0 z-50 border-b border-border/60 bg-background/95 backdrop-blur-xl">
        <div className="page-container flex h-12 min-w-0 items-center md:h-14 lg:grid lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)]">
          <a href="/" aria-label={`${brand.displayName} home`} className="mr-4 shrink-0 lg:mr-0 lg:justify-self-start"><Brand className="w-[112px] sm:w-[142px]" label={brand.displayName} /></a>
          <div className="public-header-rail hidden min-w-0 lg:block lg:justify-self-center">
            <div className="flex h-full min-w-max items-center justify-center gap-8 px-1">
              {links.map((link) => {
