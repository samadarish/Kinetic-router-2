...
import type { PortalUser, SessionView } from '@kineticrouter/portal-contract';
...
export function ProfilePage() {
  ...
  const updateProfile = useMutation({ mutationFn:()=>portalApi<PortalUser>('/me',{method:'PATCH',...jsonBody({username,avatarUrl:avatarUrl || null})}), onSuccess:(updatedUser)=>{setNotice('Profile updated.');client.setQueryData(['profile'],updatedUser);client.setQueryData<SessionView>(['session'],(current)=>current?.authenticated?{...current,user:updatedUser}:current);} });
  ...
  return <>
    ...
    <div className="profile-layout">
      ...
      <section className="profile-sections">
        ...
        <Card className="settings-card muted-setting"><header><span><Bell size={18}/></span><div><h2>Account bindings</h2><p>Email notifications and sign-in options depend on the features enabled for your account.</p></div></header><div className="binding-row"><span><Mail size={16}/>Email address</span><strong>{user.email}</strong><Badge tone={user.emailBound===false?'neutral':'success'}>{user.emailBound===false?'Unbound':'Bound'}</Badge></div><div className="binding-row disabled"><span><ShieldCheck size={16}/>Authenticator & passkeys</span><strong>{capabilities?.totp||capabilities?.passkeys?'Available in account settings':'Not enabled'}</strong><Badge>{capabilities?.totp||capabilities?.passkeys?'Available':'Unavailable'}</Badge></div></Card>
      </section>
    </div>
