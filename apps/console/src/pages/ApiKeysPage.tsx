...
import { apiKeyGroupLabel, isKeyGroupSelectionValid, keyGroupPayload } from '../lib/api-key-group';
...
export function ApiKeysPage() {
  ...
  const noGroupsAvailable = groups.isSuccess && groups.data.length === 0;
  ...
  return <>
    <PageHeader title="API Keys" description="Create and manage keys that authenticate requests to kineticRouter." action={capabilities?.keyWrites && groups.isSuccess ? <Button disabled={noGroupsAvailable} onClick={() => { setNotice(''); setEditor('create'); }}><Plus size={16} /> Create API key</Button> : undefined} />
    ...
    {capabilities?.keyWrites && noGroupsAvailable && <div className="info-banner"><ShieldCheck size={17} /><div><strong>No groups available</strong><span>Contact support to get access to a group before creating an API key.</span></div></div>}
    ...
    <Card className="table-card" aria-busy={query.isFetching || resultsPending}>
      {query.isLoading ? <LoadingState label="Loading API keys" /> : query.error ? <ErrorState error={query.error} retry={() => void query.refetch()} /> : !query.data?.items.length ? <EmptyState title="No API keys yet" description="Create a key to start making requests through api.kineticrouter.com/v1." action={capabilities?.keyWrites && groups.isSuccess ? <Button disabled={noGroupsAvailable} onClick={() => { setNotice(''); setEditor('create'); }}><Plus size={16} /> Create your first key</Button> : undefined} /> : <>
        <div className="data-table-wrap">
          <table className="data-table keys-table">
            ...
            <tbody>{query.data.items.map((key) => <tr key={key.id}>
              ...
              <td>{apiKeyGroupLabel(key)}</td><td><StatusBadge status={key.status} /></td>
    ...
    <KeyEditor key={editor === 'create' ? 'create' : editor?.id ?? 'closed'} open={Boolean(editor)} existing={editor === 'create' ? undefined : editor ?? undefined} groups={groups.data ?? []} canWrite={Boolean(capabilities?.keyWrites && groups.isSuccess)} onClose={() => setEditor(null)} onSaved={async () => { setEditor(null); setNotice(editor === 'create' ? 'API key created.' : 'API key updated.'); await refreshKeys(); }} />
...
export function KeyEditor({ open, existing, groups, canWrite, onClose, onSaved }: { open: boolean; existing?: ApiKey; groups: Group[]; canWrite: boolean; onClose(): void; onSaved(): void }) {
  ...
  const validGroup = isKeyGroupSelectionValid(form.groupId, groups, existing?.groupId);
  const currentGroupUnavailable = Boolean(existing?.groupId && !groups.some(group => group.id === existing.groupId));
  const mutation = useMutation({
    mutationFn: () => {
      if (!canWrite) throw new Error('API key changes are temporarily unavailable.');
      const group = keyGroupPayload(form.groupId, groups, existing?.groupId);
      return portalApi(existing ? `/api-keys/${encodeURIComponent(existing.id)}` : '/api-keys', {
        method: existing ? 'PATCH' : 'POST',
        ...jsonBody({ ...(existing ? updatePayload(form) : createPayload(form)), ...group }),
      });
    },
    onSuccess: onSaved,
  });
  ...
  function submit(event: FormEvent) { event.preventDefault(); if (canWrite && validGroup && !mutation.isPending) mutation.mutate(); }
  return <Modal open={open} title={existing ? 'Edit API key' : 'Create API key'} onClose={onClose} footer={<><Button variant="secondary" onClick={onClose}>Cancel</Button><Button disabled={mutation.isPending || !canWrite || !validGroup} onClick={() => document.getElementById('key-editor-submit')?.click()}>{mutation.isPending ? 'Saving…' : existing ? 'Save changes' : 'Create key'}</Button></>}>
    <form className="key-form" onSubmit={submit}>
      ...
      <label className="field"><span>Group</span><select className="field-select" value={form.groupId} onChange={(event) => update('groupId', event.target.value)} required><option value="" disabled>Select a group</option>{currentGroupUnavailable && existing && <option value={existing.groupId!}>{apiKeyGroupLabel(existing)} (current group)</option>}{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label>
      {!groups.length && !existing?.groupId && <p className="field-hint">No groups are available. Contact support to get access to a group before saving this key.</p>}
      {currentGroupUnavailable && <p className="field-hint">You can keep the current group or select an available group.</p>}
