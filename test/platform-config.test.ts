...
describe('shared product configuration', () => {
  ...
  it('allows only canonical console return paths and maps legacy links', () => {
    ...
    expect(resolveConsoleReturnPath('/support?ticket=abc')).toBe('/support?ticket=abc');
    expect(resolveConsoleReturnPath('/admin/support?ticket=abc')).toBe('/admin/support?ticket=abc');
