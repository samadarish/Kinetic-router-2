...
function CodexCard() {
  ...
  return (
    <article className="surface flex min-h-[460px] flex-col p-5 md:p-7">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3 rounded-lg border border-border px-3 py-2.5 text-[10px] text-muted-foreground"><span className="font-semibold">Config</span><code className="min-w-0 flex-1 truncate font-mono">~/.codex/config.toml</code><button aria-label="Copy Codex config" onClick={() => copy(config)}><ClipboardIcon className="h-4 w-4" /></button><a href="/docs/integrations/codex/websocket" className="inline-flex items-center gap-1 rounded-md border border-border px-3 py-1.5 text-foreground">View partial guide <ExternalIcon className="h-3 w-3" /></a>{copied && <span className="text-primary">Copied</span>}</div>
    </article>
  );
}
  ...
  return (
    <article className="surface flex min-h-[460px] flex-col overflow-hidden">
      <div className="flex items-center gap-2 overflow-auto border-b border-border p-3">
        ...
        <span className="inline-flex shrink-0 items-center gap-2 rounded-md border border-border px-3 py-1.5 text-[11px]"><ProviderLogo provider="claude" className="h-3.5 w-3.5" />Claude Opus 4.8 <span className="text-[8px] text-muted-foreground">PLANNED</span></span>
...
const popular = [
  { provider: 'Anthropic', providerId: 'anthropic', status: 'Planned', name: 'Claude Opus 5', id: 'anthropic/claude-opus-5', rate: '0.2x', input: '$1/M', output: '$5/M', cache: '$0.1/M', oi: '$5/M', oo: '$25/M', oc: '$0.5/M' },
  { provider: 'OpenAI', providerId: 'openai', status: 'Partial', name: 'GPT-5.6 Sol', id: 'openai/gpt-5.6-sol', rate: '0.15x', input: '$0.6/M', output: '$3/M', cache: '$0.06/M', oi: '$4/M', oo: '$20/M', oc: '$0.4/M' },
  { provider: 'xAI', providerId: 'grok', status: 'Planned', name: 'Grok 4.6', id: 'grok/grok-4.6', rate: '0.15x', input: '$0.3/M', output: '$0.9/M', cache: '$0.075/M', oi: '$2/M', oo: '$6/M', oc: '$0.5/M' },
];
