...
import { providerAllowsInteraction, providerAvailability, providerDisplayStatus } from '@/data/provider-availability';
...
client = OpenAI(
    base_url="${providerAvailability.openai.baseUrl}",
...
response = client.chat.completions.create(
  ...
  Anthropic: { provider: 'anthropic', code: '' },
  Grok: { provider: 'grok', code: '' },
} as const;
...
export function HomeHero() {
  ...
  const selectedStatus = providerDisplayStatus(selected.provider);
  ...
  const available = providerAllowsInteraction(selected.provider);
  ...
  return (
    <section className="hero-grid hero-glow relative overflow-hidden pb-10 pt-16 md:pb-16 md:pt-20">
      <div className="relative z-10 mx-auto max-w-5xl px-4 text-center">
        ...
        <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-muted-foreground">Current API status: OpenAI {openAiStatus.badgeLabel}, Anthropic {anthropicStatus.badgeLabel}, and Grok {grokStatus.badgeLabel}. Browse the exact Hao.ai reference catalog with each route state clearly marked.</p>
        <div aria-label="Model providers and route status" className="mt-5 flex flex-wrap items-center justify-center gap-2.5">
          {(Object.entries(examples) as [Provider, (typeof examples)[Provider]][]).map(([name, item]) => { const status = providerDisplayStatus(item.provider); return <a key={name} href={`/models/${item.provider}`} className="inline-flex h-10 items-center gap-2 rounded-full border border-border bg-card/75 px-4 text-sm font-semibold text-muted-foreground shadow-lg"><ProviderLogo provider={item.provider} className="h-4 w-4" />{name}<span className="text-[8px] tracking-wider opacity-65">{status.badgeLabel.toUpperCase()}</span></a>; })}
        </div>
        ...
        <div className="mx-auto mt-8 max-w-xl overflow-hidden rounded-lg border border-border bg-[var(--code)] text-left shadow-2xl">
          <div className="flex h-10 items-end border-b border-border/70 px-3">
            {(Object.keys(examples) as Provider[]).map((item) => { const status = providerDisplayStatus(examples[item].provider); return <button key={item} onClick={() => { setProvider(item); setCopied(false); }} className={`inline-flex h-10 items-center gap-1.5 border-b-2 px-3 text-[11px] font-medium ${provider === item ? 'border-primary text-foreground' : 'border-transparent text-muted-foreground'}`}><ProviderLogo provider={examples[item].provider} className="h-3.5 w-3.5" />{item}<span className="text-[7px] opacity-60">{status.badgeLabel.toUpperCase()}</span></button>; })}
            <button aria-label={available ? 'Copy code' : `${selectedStatus.label} example is ${selectedStatus.badgeLabel} and cannot be copied`} disabled={!available} onClick={copy} className="ml-auto flex h-10 items-center gap-1 px-2 text-[10px] text-muted-foreground hover:text-foreground disabled:cursor-not-allowed disabled:opacity-35"><ClipboardIcon className="h-3.5 w-3.5" />{copied && 'Copied'}</button>
          </div>
          {available ? <pre className="h-[278px] overflow-auto p-4 font-mono text-[12px] leading-[1.75] text-[#34332f] dark:text-[#e8e6e3] sm:h-[292px] sm:text-[13px]"><code>{selected.code}</code></pre> : <div className="grid h-[278px] place-items-center p-8 text-center sm:h-[292px]"><div><ProviderLogo provider={selected.provider} className="mx-auto h-9 w-9" /><strong className="mt-4 block text-sm">{selectedStatus.protocolLabel} route is {selectedStatus.badgeLabel.toLowerCase()}</strong><p className="mx-auto mt-2 max-w-sm text-xs leading-6 text-muted-foreground">{selectedStatus.summary} {selectedStatus.evidenceNote}</p></div></div>}
        </div>
