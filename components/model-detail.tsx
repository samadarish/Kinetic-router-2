    ...
    <>
      <section className="border-b border-border/60 py-10 md:py-14">
        <div className="page-container">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-[1fr_360px]">
            ...
            <aside className="surface p-5"><div className="flex items-center"><span className="text-xs font-semibold">Model ID</span>{rate != null && <span className="ml-auto rounded-md border border-[var(--brand-soft-border)] bg-[var(--brand-soft-bg)] px-2 py-1 text-[10px] font-semibold text-[var(--brand-soft-text)]">{Number(rate.toFixed(2))}x snapshot</span>}</div><code className="mt-3 block break-all rounded-lg bg-muted p-3 font-mono text-[11px]">{model.id}</code><div className="mt-5 grid grid-cols-2 gap-4 border-t border-border pt-5 text-xs"><div><span className="block text-muted-foreground">Upstream context</span><strong className="mt-1 block text-lg">{formatTokens(displayContextWindow(model))}</strong></div><div><span className="block text-muted-foreground">Upstream output</span><strong className="mt-1 block text-lg">{modelLimit(model.upstreamMaxOutput, model.maxOutput)}</strong></div><div><span className="block text-muted-foreground">Snapshot gateway input</span><strong className="mt-1 block">{formatTokens(model.gatewayMaxInput ?? model.contextWindow)}</strong></div><div><span className="block text-muted-foreground">Snapshot gateway output</span><strong className="mt-1 block">{formatTokens(model.gatewayMaxOutput ?? model.maxOutput)}</strong></div></div>{canInteract ? <a href="https://console.kineticrouter.com/sign-in" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 py-3 text-xs font-semibold text-white">Get API key <ArrowRightIcon className="h-4 w-4" /></a> : <span className="mt-5 inline-flex w-full items-center justify-center rounded-lg border border-border px-4 py-3 text-xs font-semibold text-muted-foreground">{modelStateLabel} catalog</span>}</aside>
          </div>
        </div>
      </section>
      <section className="page-container grid gap-10 py-10 lg:grid-cols-[minmax(0,1fr)_280px] lg:py-14">
        <div className="min-w-0">
          ...
          <h2 className="mt-12 text-2xl font-semibold tracking-tight">API example</h2><p className="mt-2 text-xs text-muted-foreground">{canInteract ? `Use the ${providerStatus.protocolLabel} Kinetic Router route with ${model.name.replace(/^[^:]+:\s*/, '')}.` : providerStatus.summary}</p><CodeExamples model={model} />
