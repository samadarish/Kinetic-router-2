...
The display registry is maintained in `web/data/provider-availability.json`; `portal/apps/web/src/lib/provider-display-status.generated.json` is its checked-in generated copy. From `web/`, run `npm run sync:provider-availability` after an operator has enabled and verified a route in Sub2API. Changing either display file does not enable a provider.
